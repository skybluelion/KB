...
public class MemberController {
	...
	public String login(HttpServletRequest request,Model model, MemberVO pvo) throws Exception{
		
		MemberVO rvo = memberService.login(pvo);
		if(rvo!=null) {
			request.getSession().setAttribute("vo", rvo); // login, update는 반드시 session에 값을 바인딩
			return "login_result";
		}else {
	...
	public String update(HttpSession session, MemberVO pvo) throws Exception {
		...
		//로그인된 상태에서만 수정 가능. Session, request 모두 가능
	...
	public String idExist(String id, Model model) throws Exception {
		...
		boolean check = false;
		if(rid!=null)check = true; //DB에 id가 존재하면 check는 true
